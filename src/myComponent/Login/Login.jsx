...
const Login = ({handleLogin}) => {
    ...
    const handleToVerify =async(e)=>{
        e.preventDefault();
         
        const formData = new FormData(e.target);

        const response = await fetch("http://localhost/IdCard/Login.php",{
            method : "POST",
            body : formData
        });
  ...
  return (
    <div className='fixed inset-0 backdrop-blur-sm z-50 flex items-center justify-center h-full w-full bg-black/50 '>
        <div className='relative'>
            ...
            <form onSubmit={handleToVerify} className='border rounded-lg p-5 flex flex-col gap-5 bg-gray-300 w-96'>
